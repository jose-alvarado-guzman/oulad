...
Two notebooks live in `notebooks/`. `oulad_data_load.ipynb` runs the ETL; `aga_student_cohorts.ipynb` opens an Aura Graph Analytics session over the loaded graph (node similarity → Louvain cohorts → outcome cross-tab → degree centrality → write-back), with dependencies in `requirements-aga.txt` and `graphdatascience` pinned exactly because its session API is still in alpha. A session is billed compute separate from AuraDB, so it carries a 2-hour TTL and a delete step.
